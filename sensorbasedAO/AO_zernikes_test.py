...
class AO_Zernikes_Test(QObject):
    ...
    def run2(self):
        try:
            ...
            """
            Scan across synthetic image with a number of scan points and a pre-determined amount of astigmatism
            """
            # Initialise AO information parameter
            self.AO_info = {'zern_test': {}}
            ...
            # Initialise arrays for storing Strehl ratios, rms wavefront errors and loop numbers
            self.strehl = np.zeros([2, config['zern_test']['scan_num_x'] * config['zern_test']['scan_num_y']])
            self.loop_rms_zern, self.loop_rms_zern_part = (np.zeros([config['AO']['loop_max'] + 1, \
                config['zern_test']['scan_num_x'] * config['zern_test']['scan_num_y']]) for i in range(2))
            self.loop_num = np.zeros(config['zern_test']['scan_num_x'] * config['zern_test']['scan_num_y'])
            j = 0

            # Generate relevant amounts of tip/tilt for scanning across synthetic image
            tilt_array = -np.linspace(-config['zern_test']['tilt_amp'], config['zern_test']['tilt_amp'], \
                config['zern_test']['scan_num_x'])
            tip_array = -np.linspace(-config['zern_test']['tip_amp'], config['zern_test']['tip_amp'], \
                config['zern_test']['scan_num_y'])

            self.message.emit('\nProcess started for scan across image with initial astigmatism...')

            prev1 = time.perf_counter()

            # Scan each point across image with a pre-determined amount of astigmatism
            for l in range(config['zern_test']['scan_num_y']):

                for k in range(config['zern_test']['scan_num_x']):

                    for i in range(config['AO']['loop_max'] + 1):
                    
                        if self.loop:

                            print('On scan point', l * config['zern_test']['scan_num_x'] + k + 1)
                                ...
                                if config['dummy']:
                                    ...
                                    if i == 0:
                                        ...
                                        if config['real_zernike']:
                                            ...
                                            self.zern_coeff[2] = config['zern_test']['zern_amp']
    ...
    def run3(self):
        try:
            ...
            self.message.emit('\nProcess started for scan across image with initial coma...')
                                ...
                                if config['dummy']:
                                    ...
                                    if i == 0:
                                        ...
                                        if config['real_zernike']:
                                            ...
                                            self.zern_coeff[0] = tip_array[l]
                                            self.zern_coeff[1] = tilt_array[k]
                                            self.zern_coeff[6] = config['zern_test']['zern_amp']
