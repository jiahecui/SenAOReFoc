...
def SH_spot_sim(settings):
    ...
    offset_coord = settings['act_ref_cent_coord']
    ...
    for i in range(len(offset_coord)):
        ...
        Gaus_spot = dirac_function(x, y, xc, yc, sigma, array_size)

        # Integrate it to pre-initialised spot image array
        A_start = [0, 0]
        B_start = [offset_coord[i] // settings['sensor_width'], offset_coord[i] % settings['sensor_width']]
        B_end = [offset_coord[i] // settings['sensor_width'] + array_size, offset_coord[i] % settings['sensor_width'] + array_size]
        SH_spot_img = array_integrate(Gaus_spot, SH_spot_img, A_start, B_start, B_end)

    return SH_spot_img

def dirac_function(x, y, xc, yc, sigma, size):
    """
