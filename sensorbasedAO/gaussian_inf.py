...
def inf(xx, yy, xc, yc, j, act_diam):
    ...
    Fn = np.exp(np.log(config['DM']['coupling_fac']) * ((xx - xc[j]) ** 2 + (yy - yc[j]) ** 2) / act_diam ** 2)
...
def inf_diff(xx, yy, xc, yc, j, act_diam, x_flag = True):
    ...
    for y in yy:
        for x in xx:
            ...
            expo = np.exp(np.log(config['DM']['coupling_fac']) * ((x - xc[j]) ** 2 + (y - yc[j]) ** 2) / act_diam ** 2)

            # Calculate x or y derivative of Gaussian distribution influence function and get final result
            if x_flag:
                dFn_dx = 2 * np.log(config['DM']['coupling_fac']) / act_diam ** 2 * (x - xc[j])
                dFn = dFn_dx * expo
            else:
                dFn_dy = 2 * np.log(config['DM']['coupling_fac']) / act_diam ** 2 * (y - yc[j])
                dFn = dFn_dy * expo
